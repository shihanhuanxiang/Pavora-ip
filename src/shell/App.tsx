...
const App: React.FC<AppProps> = ({ taxonomyData }) => {
  ...
  /*
   * 2026-08-14（階段 7 · A3）：這裡原本有一個掛載後執行的 effect，
   * 呼叫 `migrateAmbassadorsFromBrandStore()` 把舊的品牌代言人併進 Model。
   * 代言人概念整個移除後那個遷移沒有意義了，effect 與 store 方法一併刪除。
   * 舊的 localStorage key（`pavora-brand-store`、`pavora-b8-ambassador-migrated`）
   * 刻意保留不清 —— 現在沒有讀取端，動存量資料的風險大於留著它們。
   */
