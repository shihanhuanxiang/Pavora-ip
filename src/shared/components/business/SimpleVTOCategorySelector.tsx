            ...
            <div className="grid grid-cols-1 gap-3">
                {VTO_CATEGORIES.map(cat => (
                    <div key={cat.id} className="space-y-2">
                        <Button 
                            variant="secondary" 
                            className={`w-full justify-start py-4 px-6 text-lg font-bold tracking-widest border-2 ${selectedCategory === cat.id ? 'border-[var(--color-gold)] bg-gold/10 text-[var(--color-gold)]' : 'border-white/5'}`}
                            ...
                            <div className="grid grid-cols-3 gap-2 pl-4">
                                {cat.subCategories.map(sub => (
                                    <button
                                        ...
                                        className="py-2 px-3 bg-white/5 hover:bg-gold/20 border border-white/10 rounded-md text-sm text-gray-300 hover:text-[var(--color-gold)] transition-all"
                                    >
                                        {sub}
                                    </button>
                                ))}
