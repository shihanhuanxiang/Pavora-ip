        ...
        <div className="container mx-auto p-4 lg:p-8 max-w-[1600px] animate-fade-in pb-24">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                ...
                <div className="lg:col-span-3 flex flex-col gap-6 h-full">
                    <Card className={`border-[var(--color-border)] flex flex-col ${!analysis ? 'flex-grow' : ''}`}>
                        <div className="flex justify-between items-center mb-5">
                            <h3 className="text-2xl font-bold text-[var(--color-text-title)]">1. 商品角度錨點</h3>
                            {uploadedCount > 0 && (
                                <button 
                                    type="button"
                                    onClick={clearAll} 
                                    className="text-xs text-red-400 hover:text-red-300 font-bold uppercase tracking-widest px-2 py-1 bg-red-900/20 rounded border border-red-900/30 transition-colors"
                                >
                                    全部刪除
                                </button>
                            )}
                            ...
                            {sourceImages.map((img, index) => (
                                <div key={index} className="flex flex-col gap-2">
                                    <div 
                                        ...
                                        className={`relative aspect-square rounded-lg border-2 flex flex-col items-center justify-center transition-all group overflow-hidden ${img ? 'border-[var(--color-border)] bg-[var(--color-bg-deep)]/40' : 'border-dashed border-[var(--color-border)] hover:border-[var(--color-gold)] bg-[var(--color-bg-surface)]/20 cursor-pointer'}`}
                                    >
                                        {img ? (
                                            <>
                                                ...
                                                <button 
                                                    ...
                                                    className="absolute top-1 right-1 bg-[var(--color-bg-deep)]/60 text-[var(--color-text-title)] p-1 rounded-full hover:bg-red-600 transition-colors"
                                                >
                                            ...
                                            <>
                                                <PhotoIcon className="w-8 h-8 text-[var(--color-text-dim)] mb-1" />
                                                <span className="text-[10px] text-[var(--color-text-dim)] font-bold uppercase">{DEFAULT_ANGLES[index]}</span>
                                            </>
                                        )}
                                    </div>
                                    <input 
                                        ...
                                        placeholder="角度標籤"
                                        className="bg-transparent border-b border-[var(--color-border)] text-[11px] text-center text-[var(--color-text-dim)] focus:outline-none focus:border-[var(--color-gold)] transition-colors py-1"
                                    />
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="space-y-2 mb-6">
                            <label className="block text-[10px] font-bold text-[var(--color-text-dim)] uppercase tracking-widest">
                                商品詳細規格 (選填)
                            </label>
                            <textarea 
                                ...
                                className="w-full bg-[var(--color-bg-surface)]/60 border border-[var(--color-border)] rounded-lg p-3 text-sm text-[var(--color-text-main)] placeholder-[var(--color-text-dim)]/50 focus:outline-none focus:border-[var(--color-gold)] transition-all resize-none"
                                rows={4}
                            />
                        </div>

                        <div className="space-y-4 mb-6">
                            <div className="flex items-center justify-between p-3 bg-[var(--color-bg-surface)]/40 rounded-lg border border-[var(--color-border)]">
                                <div className="flex flex-col">
                                    <span className="text-sm font-bold text-[var(--color-text-main)]">產出獨立圖片</span>
                                    <span className="text-[10px] text-[var(--color-text-dim)]">嚴格順序生成模式</span>
                                </div>
                                <input 
                                    type="checkbox" 
                                    checked={isBatchMode} 
                                    onChange={(e) => setIsBatchMode(e.target.checked)}
                                    className="w-6 h-6 rounded bg-[var(--color-bg-surface)] border-[var(--color-border)] text-[var(--color-gold)] focus:ring-[var(--color-gold)]"
                                />
                            </div>

                            <Select 
                                label="生成品質 (Rendering Quality)" 
                                options={[
                            ...
                            <Select 
                                label="品牌氛圍 (Brand Atmosphere)" 
                                options={[
                                    { value: 'default', label: '自動匹配 (Auto)' },
                                    { value: 'luxury', label: '奢華黑金 (Luxury)' },
                                    { value: 'minimalist', label: '北歐極簡 (Minimalist)' },
                                    { value: 'vibrant', label: '活力色彩 (Vibrant)' },
                                    { value: 'tech', label: '硬核科技 (Tech)' }
                                ]}
                        ...
                        <Button 
                            ...
                            className="w-full text-xl py-5 shadow-2xl"
                        >
                    ...
                    {analysis && (
                        <Card className="border-[var(--color-border)] bg-[var(--color-bg-surface)]/40 animate-fade-in flex-grow">
                            <h3 className="text-xl font-bold text-[var(--color-gold)] mb-5 flex items-center gap-2"><SparklesIcon className="w-5 h-5"/> 核心特徵解構</h3>
                            <div className="space-y-5">
                                <div><span className="text-xs font-bold text-[var(--color-text-dim)] uppercase tracking-widest block mb-1">鎖定品類</span><p className="text-[var(--color-text-main)] text-base font-medium">{analysis.basic_info?.color} {analysis.basic_info?.category}</p></div>
                                <div><span className="text-xs font-bold text-[var(--color-text-dim)] uppercase tracking-widest block mb-1">材質 DNA</span><p className="text-[var(--color-text-main)] text-base font-medium">{analysis.basic_info?.material}</p></div>
                                <div><span className="text-xs font-bold text-[var(--color-text-dim)] uppercase tracking-widest block mb-1">光學屬性</span><p className="text-[var(--color-gold)] text-sm font-bold">{analysis.basic_info?.optical || '自動分析中...'}</p></div>
                                {style && (
                                    <>
                                        <div className="pt-2 border-t border-[var(--color-border)]/30">
                                            <span className="text-xs font-bold text-[var(--color-text-dim)] uppercase tracking-widest block mb-1">建議佈光</span>
                                            <p className="text-[var(--color-text-main)] text-xs leading-relaxed">{style.lighting_protocol}</p>
                                        </div>
                                        <div>
                                            <span className="text-xs font-bold text-[var(--color-text-dim)] uppercase tracking-widest block mb-1">材質響應</span>
                                            <p className="text-[var(--color-text-main)] text-xs leading-relaxed">{style.material_response}</p>
                                        </div>
                                    </>
                                )}
                                <div><span className="text-xs font-bold text-[var(--color-text-dim)] uppercase tracking-widest block mb-2">關鍵特徵</span><ul className="space-y-2">{analysis.physical_traits?.map((p: string, i: number) => <li key={i} className="text-sm text-[var(--color-text-dim)] flex gap-2"><span className="text-[var(--color-gold)]">•</span>{p}</li>)}</ul></div>
                            </div>
                        </Card>
                    )}
                </div>
                ...
                <div className="lg:col-span-9">
                    <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8 border-b border-[var(--color-border)] pb-6">
                        <div className="flex flex-col">
                            <h3 className="text-2xl font-display font-bold uppercase tracking-[0.2em] text-[var(--color-text-title)]">視覺輸出矩陣</h3>
                            <span className="text-[9px] uppercase tracking-[0.4em] text-[var(--color-gold)] font-light">E-Gen Production Matrix</span>
                        </div>
                        {batchPosters.some(p => p.imageUrl) && (
                            <Button onClick={handleDownloadAllBatch} variant="secondary" className="text-sm font-bold px-6">
                                <DownloadIcon className="w-5 h-5 mr-2" /> 批次下載全案素材
                            </Button>
                        )}
                        {singleGridResult && (
                             <Button onClick={() => downloadImage(singleGridResult, 'pavora_egen_lookbook.jpg', 'EGen')} variant="secondary" className="text-sm font-bold px-6">
                    ...
                    {!isBatchMode ? (
                        <div className="flex justify-center items-start pt-4">
                            {singleGridResult ? (
                                <div className="w-full max-w-2xl group relative bg-[var(--color-bg-surface)] p-4 rounded shadow-2xl animate-fade-in">
                                    ...
                                    <div className="mt-4 text-center">
                                        <span className="text-[10px] text-[var(--color-text-dim)] uppercase tracking-[0.4em]">Pavora Automated Lookbook v3.1</span>
                                    </div>
                                </div>
                            ) : (
                                <div className="h-[60vh] w-full flex flex-col items-center justify-center border-4 border-dashed border-[var(--color-border)] rounded-3xl text-[var(--color-text-dim)] bg-[var(--color-bg-surface)]/10">
                                    <PhotoIcon className={`w-32 h-32 mb-6 ${isLoading ? 'animate-pulse text-[var(--color-gold)]' : 'opacity-10'}`} />
                                    <p className="text-2xl font-black text-[var(--color-text-dim)] uppercase tracking-tighter">
                                        {isLoading ? 'RENDERING GRID...' : 'Grid Standby'}
                                    </p>
                                    <p className="text-sm mt-3 text-center px-12 text-[var(--color-text-dim)]">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                            {batchPosters.length > 0 ? batchPosters.map(poster => (
                                <Card key={poster.id} className="p-0 flex flex-col bg-[var(--color-bg-surface)] border-[var(--color-border)] overflow-hidden hover:border-[var(--color-gold)]/50 transition-all group shadow-xl rounded-2xl">
                                    <div className="aspect-[9/16] bg-[var(--color-bg-deep)]/40 relative flex items-center justify-center">
                                        {poster.status === 'loading' && (
                                            <div className="flex flex-col items-center gap-4">
                                                ...
                                                <span className="text-xs text-[var(--color-text-dim)] font-bold uppercase tracking-widest">Rendering...</span>
                                            </div>
                                        )}
                                        {poster.status === 'success' && poster.imageUrl && (
                                            <>
                                                ...
                                                <div className="absolute top-4 left-4 bg-[var(--color-bg-deep)]/70 text-[var(--color-gold)] text-[10px] px-3 py-1.5 rounded-lg border border-[var(--color-gold)]/30 backdrop-blur-md font-black shadow-lg">
                                                    {poster.title_zh}
                                                </div>
                                            </>
                                        )}
                                        {poster.status === 'error' && (
                                            <div className="flex flex-col items-center gap-4 p-6">
                                                <div className="text-red-500 text-sm font-bold">渲染失敗</div>
                                                <button 
                                                    type="button"
                                    ...
                                    <div className="p-6 bg-[var(--color-bg-surface)]/90 border-t border-[var(--color-border)] flex-grow">
                                        {poster.copy ? (
                                            <div className="space-y-4">
                                                <h4 className="text-[var(--color-text-title)] font-bold text-lg leading-tight">{poster.copy.title}</h4>
                                                <p className="text-xs text-[var(--color-gold)] font-bold uppercase tracking-wider opacity-80">{poster.copy.subtitle}</p>
                                                <div className="pt-4 border-t border-[var(--color-border)]/50 space-y-1.5">
                                                    {poster.copy.bullets.map((b, i) => (
                                                        <p key={i} className="text-[11px] text-[var(--color-text-dim)] flex gap-2"><span>•</span>{b}</p>
                                                    ))}
                                                </div>
                                            </div>
                                        ) : <div className="h-20 flex items-center justify-center text-[var(--color-text-dim)] text-xs italic">等待文案生成...</div>}
                                    </div>
                                </Card>
                            )) : (
                                <div className="col-span-full h-[60vh] flex flex-col items-center justify-center border-4 border-dashed border-[var(--color-border)] rounded-3xl text-[var(--color-text-dim)] bg-[var(--color-bg-surface)]/10">
                                    <EGenIcon className="w-32 h-32 mb-6 opacity-10" />
                                    <p className="text-2xl font-black text-[var(--color-text-dim)] uppercase tracking-tighter">Batch Mode Ready</p>
                                    <p className="text-sm mt-3 text-center px-12 text-[var(--color-text-dim)]">點擊開始生成，系統將自動按順序產出 9 張高品質素材</p>
                                </div>
                            )}
                        </div>
                    )}
