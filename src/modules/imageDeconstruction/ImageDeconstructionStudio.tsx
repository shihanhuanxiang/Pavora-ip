        ...
        <div className="container mx-auto p-8 max-w-7xl animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div><Card className="h-full p-0 overflow-hidden">
                    <div className="p-4 border-b border-[var(--color-border)] bg-bg-surface/50 flex justify-between items-center">
                        <div className="flex flex-col">
                    ...
                    <div className="p-6">
                        {image ? ( <div className="relative group w-full"><img src={image.url} alt="Input" className="w-full max-h-[60vh] object-contain rounded-md" /><div className="absolute inset-0 bg-bg-deep/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2"><Button onClick={() => fileInputRef.current?.click()}>更換圖片</Button></div></div> ) : ( <div onClick={() => fileInputRef.current?.click()} className="w-full bg-bg-input/50 border-2 border-dashed border-[var(--color-border)] rounded-lg p-16 text-center cursor-pointer hover:border-[var(--color-gold)] flex flex-col items-center justify-center"><PhotoIcon className="w-16 h-16 text-[var(--color-text-dim)] mb-4" /><p className="text-lg text-[var(--color-text-dim)]">點擊上傳圖片</p></div> )}
