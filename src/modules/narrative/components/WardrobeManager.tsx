...
export const STYLE_ARCHETYPE_MAP: Record<string, string> = {
    'feminine_sweet': '甜美少女風格',
    'feminine_mature': '成熟御姐風格',
    'korean_chic': '韓系時髦風格',
    'street_edgy': '街頭辣妹風格',
    'sporty_active': '運動元氣風格',
    'vintage_retro': '復古懷舊風格',
    'minimalist': '極簡清爽風格',
    'tomboy': '率性中性風格',
    'masculine_clean': '清爽男友風格',
    'masculine_rugged': '硬朗戶外風格',
    'masculine_formal': '都會正裝風格',
    'feminine_office': '職場精英風格',
    'dandy_refined': '雅痞紳士風格',
    'cultural_traditional': '文化深蘊風格',
    'street_techwear': '街頭機能風格'
};
        ...
        <div className="space-y-8 max-w-7xl mx-auto px-4 sm:px-6">
            ...
            >
                <div className="space-y-4">
                    ...
                    {/* View Filters */}
                    <div className="flex bg-narrative-mist/50 border border-narrative-ink/10 p-1 rounded-xl w-fit ml-5">
                        {[
                            { id: 'all', label: '全部' },
                            { id: 'seeds', label: '內建資源' },
                            { id: 'user', label: '自定義' }
                            ...
                            <button
                                ...
                                className={`px-4 py-1.5 text-[12px] font-black uppercase tracking-widest rounded-lg transition-all ${
                                    viewMode === mode.id
                                    ? 'bg-[var(--color-wine)] text-white shadow-lg shadow-wine/20'
                                    : 'text-narrative-ink-soft hover:text-narrative-ink'
                                }`}
                            >
                                {mode.label}
                            </button>
                ...
                <button 
                    ...
                    className="group relative px-8 py-3 overflow-hidden rounded-2xl bg-[var(--color-bg-card)] text-[var(--color-text-main)] border border-[var(--color-border)] font-black text-[13px] transition-all hover:bg-[var(--color-wine)] hover:text-white hover:border-[var(--color-wine)] active:scale-95 shadow-xl"
                >
            ...
            <motion.div
                className="ml-5 px-6 py-4 bg-brass/10 border border-brass/20 rounded-2xl flex items-center gap-4 text-narrative-ink"
            >
                <div className="flex-shrink-0 w-6 h-6 rounded-full border border-brass/40 flex items-center justify-center text-[11px] font-bold text-[var(--color-brass)]">
                    i
                </div>
                <div className="flex flex-col gap-0.5">
                    <p className="text-xs font-bold tracking-wide text-narrative-ink">
                            ...
                            >
                                <div 
                                    className={`group relative bg-[var(--color-bg-card)] border rounded-[2.5rem] p-8 transition-all shadow-2xl overflow-hidden h-full flex flex-col active:scale-[0.98] cursor-pointer select-none ${
                                        isActive
                                        ? 'border-[var(--color-wine)] bg-[var(--color-bg-card)] shadow-[0_20px_50px_rgba(111,39,53,0.15)] ring-1 ring-wine/30 scale-[1.02]'
                                        : 'border-[var(--color-border)] hover:border-brass/30'
                                    }`}
                                    onClick={() => handleSelectOutfit(outfit.outfit_id)}
                                >
                                    {/* Selection Glow */}
                                    {isActive && (
                                        <div className="absolute inset-0 bg-gradient-to-br from-wine/5 to-transparent pointer-events-none"></div>
                                    )}

                                    {/* Hover Hint: Click to Lock */}
                                    {!isActive && (
                                        <div className="absolute top-16 right-6 opacity-0 group-hover:opacity-70 transition-opacity duration-300 pointer-events-none">
                                            <span className="text-[11px] font-bold tracking-widest uppercase text-gray-600 leading-none">
                                                點擊鎖定
                                            </span>
                                        </div>
                                    )}
                                    ...
                                    <div className="space-y-6 relative z-10 flex-1 pointer-events-none">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <div className="flex gap-2">
                                                <span className={`px-3 py-1 bg-[var(--color-bg-input)] border border-[var(--color-border)] text-[11px] font-black rounded-full uppercase tracking-tighter ${isActive ? 'text-[var(--color-wine)]' : 'text-gray-500'}`}>美學階層 {outfit.aesthetic_tier}</span>
                                                {isSeed && <span className="px-2 py-1 bg-steel/10 border border-steel/20 text-[var(--color-steel)] text-[11px] font-bold rounded-md uppercase tracking-tighter">系統核心</span>}
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-2 gap-4 text-[11px]">
                                            <div className="space-y-3">
                                                <div className="space-y-1">
                                                    <p className="text-gray-400 dark:text-gray-600 font-bold uppercase text-[11px] tracking-widest">上裝</p>
                                                    <p className="text-gray-800 dark:text-white font-medium line-clamp-1">{getTopDisplayText(outfit)}</p>
                                                </div>
                                                <div className="space-y-1">
                                                    <p className="text-gray-400 dark:text-gray-600 font-bold uppercase text-[11px] tracking-widest">鞋履</p>
                                                    <p className="text-gray-800 dark:text-white font-medium line-clamp-1">{getOutfitDisplayText(outfit.pillars.shoes, 'shoes')}</p>
                                                </div>
                                            </div>
                                            <div className="space-y-3">
                                                <div className="space-y-1">
                                                    <p className="text-gray-400 dark:text-gray-600 font-bold uppercase text-[11px] tracking-widest">下裝</p>
                                                ...
                                                <div className="space-y-1">
                                                    <div className="flex justify-between items-center mb-1">
                                                        <p className="text-gray-600 font-bold uppercase text-[11px] tracking-widest">物理磨損狀態</p>
                                                        <span className={`text-[11px] font-black uppercase ${
                                                        outfit.wear_state === 'worn_in' ? 'text-red-500' : 'text-[var(--color-brass)]'
                                                        }`}>
                                    ...
                                    <div className="mt-8 pt-4 border-t border-[var(--color-border)] flex items-center justify-between pointer-events-none">
                                        ...
                                        <span className="text-[11px] text-gray-400 dark:text-gray-600 font-bold uppercase tracking-[0.3em]">
                                            {outfit.gender === 'F' ? '女性模組' : outfit.gender === 'M' ? '男性模組' : '全性別適用'}
                                        </span>
                                    </div>
                                </div>
                            </div>
            ...
            {showForm && (
                <div className="fixed inset-0 bg-bg-deep/95 backdrop-blur-md z-[100] flex items-center justify-center p-4 sm:p-8">
                    <div
                    ...
                    >
                        {/* Background Accents */}
                        <div className="absolute top-0 right-0 w-64 h-64 bg-brass/5 blur-[100px] -mr-32 -mt-32"></div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-10 relative z-10 custom-scrollbar max-h-[70vh] overflow-y-auto pr-4">
                            <div className="md:col-span-8 space-y-10">
                                ...
                                <div className="space-y-6">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <h4 className="text-[11px] font-black text-white/40 uppercase tracking-[0.3em]">基礎識別</h4>
                                    </div>
                                    <div className="grid grid-cols-2 gap-6">
                                        <div className="group space-y-2">
                                            <label className="text-[11px] text-gray-500 font-bold uppercase tracking-widest group-focus-within:text-[var(--color-brass)] transition-colors">穿搭編碼</label>
                                            <input
                                                type="text"
                                                className="w-full bg-[var(--color-bg-input)] border border-[var(--color-border)] rounded-2xl px-5 py-3 text-xs text-[var(--color-text-main)] focus:border-brass/50 transition-all outline-none"
                                        ...
                                        <div className="group space-y-2">
                                            <label className="text-[11px] text-gray-500 font-bold uppercase tracking-widest">風格原型</label>
                                            <select 
                                                className="w-full bg-[var(--color-bg-input)] border border-[var(--color-border)] rounded-2xl px-5 py-3 text-xs text-[var(--color-text-main)] focus:border-brass/50 outline-none appearance-none cursor-pointer"
                                                value={newOutfit.style_archetype}
                                                onChange={e => setNewOutfit({ ...newOutfit, style_archetype: e.target.value })}
                                            >
                                                <option value="feminine_sweet">甜美少女風格</option>
                                                <option value="feminine_mature">成熟御姐風格</option>
                                                <option value="masculine_clean">清爽簡約風格</option>
                                                <option value="masculine_rugged">硬朗粗獷風格</option>
                                                <option value="masculine_formal">商務正裝風格</option>
                                                <option value="tomboy">率性中性風格</option>
                                                <option value="minimalist">極簡主義風格</option>
                                                <option value="vintage_retro">復古懷舊風格</option>
                                                <option value="street_techwear">街頭機能風格</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-6">
                                    <div className="flex items-center gap-3">
                                        <div className="w-1 h-3 bg-white/20 rounded-full"></div>
                                        <h4 className="text-[11px] font-black text-white/40 uppercase tracking-[0.3em]">穿搭四大支柱</h4>
                                    </div>
                                    <div className="grid grid-cols-2 gap-6">
                                        {[
                                            { label: '上身單品', key: 'top', placeholder: '例如：寬鬆奶油色針織衫...' },
                                            { label: '下身單品', key: 'bottom', placeholder: '例如：高腰亞麻寬褲...' },
                                            { label: '鞋履配備', key: 'shoes', placeholder: '例如：極簡皮革涼鞋...' },
                                            { label: '外層疊穿', key: 'layer_outer', placeholder: '例如：長版絲質長袍...' }
                                        ].map((pillar) => (
                                            <div key={pillar.label} className="group space-y-2">
                                                <label className="text-[11px] text-gray-500 font-bold uppercase tracking-widest">{pillar.label}</label>
                                                <input 
                                                    type="text" 
                                                    className="w-full bg-white/[0.03] border border-white/5 rounded-2xl px-5 py-3 text-xs text-white focus:border-brass/50 transition-all outline-none"
                                ...
                                <div className="space-y-6">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                            <div className="w-1 h-3 bg-white/20 rounded-full"></div>
                                            <h4 className="text-[11px] font-black text-white/40 uppercase tracking-[0.3em]">物理細節：磨損與質地</h4>
                                        </div>
                                        <span className="text-[11px] text-[var(--color-brass)] font-mono animate-pulse tracking-widest">寫實渲染引擎已啟動</span>
                                    </div>
                                    <div className="grid grid-cols-2 gap-4">
                                        {[
                                            { id: 'barely_worn', label: '幾乎全新・極致平整', desc: '極致正式' },
                                            { id: 'well_loved', label: '穿過・自然褶皺', desc: '寫實主義' },
                                            { id: 'worn_in', label: '略舊・明顯磨損', desc: '明顯磨損' }
                                        ].map(state => (
                                            <motion.button
                                                key={state.id}
                                                whileHover={{ y: -2 }}
                                                ...
                                                className={`p-5 rounded-[2rem] text-left transition-all border flex flex-col gap-2 ${
                                                    ...
                                                    ? 'bg-[var(--color-wine)]/[0.08] border-wine/40 shadow-[0_10px_25px_rgba(111,39,53,0.1)]'
                                                    : 'bg-white/[0.02] border-white/5 hover:bg-white/[0.04]'
                                                }`}
                                            >
                                                <span className={`text-[11px] font-black tracking-wider ${newOutfit.wear_state === state.id ? 'text-[var(--color-wine)]' : 'text-white'}`}>{state.label}</span>
                                                <span className="text-[11px] text-gray-500 font-bold tracking-widest uppercase">{state.desc}</span>
                                            </motion.button>
                                        ))}
                                    </div>
                                </div>
                            ...
                            <div className="md:col-span-4 space-y-10 border-l border-white/5 pl-10">
                                ...
                                <div className="space-y-6">
                                    <h4 className="text-[11px] font-black text-white/40 uppercase tracking-[0.3em]">美學階層</h4>
                                    <div className="space-y-4">
                                        {[
                                            { id: 1, label: '階層 1：日常', desc: '日常寫實' },
                                            { id: 2, label: '階層 2：流行', desc: '精緻流行' },
                                            { id: 3, label: '階層 3：攝影', desc: '商業棚拍' },
                                            { id: 4, label: '階層 4：奢華', desc: '奢華美學' },
                                            { id: 5, label: '階層 5：雜誌', desc: '雜誌封面' }
                                        ].map(tier => (
                                            <button
                                                key={tier.id}
                                                type="button"
                                ...
                                {/* Summary / Tips */}
                                <div className="p-6 bg-[var(--color-brass)]/[0.03] border border-brass/10 rounded-[2rem] space-y-4">
                                    <div className="flex items-center gap-2">
                                        <svg className="w-4 h-4 text-[var(--color-brass)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                        <span className="text-[11px] text-[var(--color-brass)] font-black uppercase tracking-widest">矩陣建議</span>
                                    </div>
                        ...
                        <div className="mt-12 pt-8 border-t border-white/5 flex gap-6 relative z-10">
                            ...
                            <button
                                onClick={handleSave}
                                className="flex-[2] py-4 bg-[var(--color-wine)] hover:bg-wine/90 text-white transition-all rounded-2xl font-black text-[13px] tracking-[0.3em] shadow-[0_20px_50px_rgba(111,39,53,0.3)] active:scale-[0.98]"
                            >
                                儲存衣櫃資產
                            </button>
                        </div>
